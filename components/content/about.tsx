...
function About() {
	...
	useEffect(() => {
		gsap.utils.toArray(".about-section").forEach((section: any, i) => {
			const wrappers = section.querySelectorAll(".about-word");

			wrappers.forEach((wrapper: any) => {
				const word = wrapper.querySelector("span");
		...
		>
			<div className="flex flex-col gap-6 md:gap-10 text-5xl md:text-6xl lg:text-8xl font-light">
				{aboutText.map((section, i) => (
					<section
						className={`about-section-${i} about-section w-full flex items-center flex-wrap justify-start gap-2 md:gap-3`}
						key={generateKey()}
					>
						{section.split(" ").map((word) => (
							<div
								key={generateKey()}
								className="overflow-wrapper-2 relative about-word"
							>
								{(() => {
									if (word === "Samridh") return <span>Samridh</span>;
									if (word === "creative")
										return (
											<GradientText
												colors={[
